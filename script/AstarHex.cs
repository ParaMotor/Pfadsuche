...
public class Astar3coords
{
	...
	public void Astar(Hex objectT, Hex start, Grid grid)
	{
		...
		UnityEngine.Debug.Log("Ende: " + objectX + ", " + objectY + ", " + objectZ + ";");
		...
		while (AstarHex.Count > 0 && abort == false)
		{
            ...
            {
				for (int j = 0; j < grid.HexList.Count; j++)
				{
					if (grid.HexList[j].xCoordinate == neighbors[i].xCoordinate && grid.HexList[j].yCoordinate == neighbors[i].yCoordinate && grid.HexList[j].zCoordinate == neighbors[i].zCoordinate)
                    {
						grid.HexList[j].ChangeColor(2);
						continue;
					}
				...
				while (zwischenX != startX && zwischenY != startY && zwischenZ != startZ && counter < 12){

					zwischenX = current.xCoordinate;
					zwischenY = current.yCoordinate;
					zwischenZ = current.zCoordinate;

					for (int j = 0; j < grid.HexList.Count; j++)
					{
						if (grid.HexList[j].xCoordinate == current.xCoordinate && grid.HexList[j].yCoordinate == current.yCoordinate && grid.HexList[j].zCoordinate == current.zCoordinate)
						{
							grid.HexList[j].ChangeColor(3);
							continue;
						}
